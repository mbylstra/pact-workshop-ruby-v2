...
describe Client do
  ...
  it 'can process the json payload from the provider' do
    ...
    expect(subject.process_data(Time.now.httpdate)).to eql([1, Time.parse(json_data['date'])])
  end
    ...
    describe "get json data" do
      ...
      before do
        our_provider.given("data count is > 0").
          ...
          with(method: :get, path: '/provider.json', query: URI::encode('date=' + date)).
          will_respond_with(
            status: 200,
      ...
      it "can process the json payload from the provider" do
        expect(subject.process_data(date)).to eql([1, Time.parse(json_data['date'])])
      end
    ...
    describe "handling invalid responses" do
      ...
      it "handles an invalid date parameter" do
        our_provider.given("data count is > 0").
          ...
          with(method: :get, path: '/provider.json', query: 'date=This%20is%20not%20a%20date').
          will_respond_with(
            status: 400,
            headers: {'Content-Type' => 'application/json'},
